...
AUX = (
    'один ряд', 'два ряда', '1 ряд', '2 ряд', '2 рчда', '3 ряда', '4 ряда', 'две границы', 'обе границы', 'пустот',
...
RE_OTHER = (
    r'\d+ под навесом$', r'навес(ом)? \d+$', r'в (серой зоне|н[еи][йи]?тралке) \d+', r'семь[я|ё|е]', r'\d+ м ',
    ...
    r'запустили\s?(\d+|\d+ - \d+)\s?машин', r'\d+\s?(лет|год|автобус|мест|сторон|процент|суток|границ|патрул|го)',
    r'\d+ (янв|февр|март|апре|мая|июн|июл|авгус|сентяб|октяб|нояб|декаб)',  r'движемся по (?:\d+ - \d+|\d+)',
    r'пеш[е|и]х(?:одов)? (около|до|бол(ьше|ее))?\s?(чем)?\s?\d+', r'бол(ьше|ее) \d+ (-?х)? часов', r'в \d+ - \d+',
    r'поворотом ещ[е|ё] (\d+ )?машин( \d+)?', r'(\d+ )?машин[ы]? (\d+ )?сразу (за)?пустили',
    r'(про|за)?пускают( по)? \d+ (авто|машин)', r'пеш[и|е]х(одов)? (нет|чел(овек)? \d+)', r'\d+( - \d+)? метров',
    r'никого не (в|вы|за)пускают',
)
DIFF = (
...
class Cleaner:
    ...
    @staticmethod
    def pre_cleaner(msg):
        ret = re.sub(r', \d+ пеш', '', msg).replace('  ', ' ')
        return ret
    ...
    def clear_rational_number(msg):
        ...
        Регулярки раделены символом |
        """
        ret = re.sub(r'\d+[,.:/]\d+|\d+/|\d+\s?%', '', msg).replace('  ', ' ')
        return ret
