...
RE_OTHER = (
    ...
    r'запустили\s?(\d+|\d+ - \d+)\s?машин', r'\d+\s?(лет|год|автобус|мест|сторон|процент|суток|границ)', r'в \d+ - \d+',
    r'\d+ (янв|февр|март|апре|мая|июн|июл|авгус|сентяб|октяб|нояб|декаб)',  r'движемся по (?:\d+ - \d+|\d+)',
    r'пеш[е|и]х(?:одов)? (около|до|бол(ьше|ее))?\s?(чем)?\s?\d+', r'бол(ьше|ее) \d+ (-?х)? часов',
)
DIFF = (
